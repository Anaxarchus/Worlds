...
func generate_offset_interfaces():
    for child in $CenterContainer/OffsetContainer.get_children():
        child.queue_free()
    var offset_count = get_offset_count()
    if skip_first:
        offset_count -= 1
    var last = null
    var next = null
    for x in range(offset_count):
        ...
        offset.offset_color = get_offset_color(x)
        
        if last == null:
            offset.set_blend_min(0.0001)
        else:
            var position = get_offset_position(last) + (get_offset_position(x) - get_offset_position(last))*.5 + 0.001
        ...
        if next == null:
            ...
            var position = get_offset_position(x) + (get_offset_position(next) - get_offset_position(x))*.5 - 0.001
